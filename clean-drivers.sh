...
delete_driver() {
    ...
    local response=$(curl -s -X DELETE "$DRIVER_API_URL/api/drivers/$driver_id" \
        -H "Authorization: Bearer $ADMIN_TOKEN")
    
    if [ $? -eq 0 ]; then
        local success=$(echo "$response" | jq -r '.success')
        if [ "$success" = "true" ]; then
            print_success "Driver $driver_id deleted successfully"
            return 0
        else
        ...
        print_error "HTTP request failed for driver $driver_id"
        return 1
    fi
}
...
# Main execution
main() {
    print_header "🧹 Driver Cleanup and Recreation"
    ...
    if [ -n "$existing_drivers" ]; then
        ...
        print_success "Deleted $count existing drivers"
    else
    ...
    print_success "Driver cleanup and recreation completed!"
